...
|----|-------------|----------------|-------------|------------|---------------|
| A01 | clean/recall.R | impute/recall.R | Food item unit conversion factors (litre/pieces → kg) | Product-specific density factors; source: literature / standard density references (no single URL — varies per item) | Review each factor against codebook; resolve NA entries before stage 3 |
| A02 | clean/crops.R | impute/crops.R | gps_area == 0 recoded to NA | LSMS team recommendation: zero GPS = unreliable | Confirm LSMS documentation; no sensitivity run needed if guidance is firm |
| A03 | clean/crops.R | impute/crops.R | GPS preferred over farmer area estimate | plotsize = GPS where available, farmer as fallback | Confirm against LSMS-ISA literature on area measurement bias |
...
| A05 | clean/crops.R | impute/crops.R | total_harvest = harvest_remain + quant_harvest | Assumes remaining harvest will be collected | Sensitivity: run with total_harvest = quant_harvest only |
...
| A07 | clean/ag_produce.R | impute/ag_produce.R | Input allocation rules (heuristic) | Cascade of if/else rules; input/quant ratios used | Review with domain expert; profile uncertain rows in exclusions audit |
...
| A13 | clean/animal_products.R | impute/animal_products.R | Three HH manual hides split | 7294-001 (1.5/1.5), 8014-001 (0/5), 4764-001 (1/1) | Re-verify if raw data changes |
...
| A18 | clean/milk.R | impute/milk.R | Three HH missing milk average filled by inspection | 1001-001 → lo; 1002-001 → midpoint; 2943-001 → sum of dispositions | Re-verify if raw data changes; see archived 04_Milk.Rmd |
| A19 | clean/milk.R | impute/milk.R | processed = psold when processed < psold | Treats sales as lower bound on processing volume | Review whether processing or sales data is more reliable in NPS4 |
...
| A24 | clean/destinations.R | impute/destinations.R | HH 8659-001 maize consumed := 480 | Manual fix — data entry error in raw | Re-verify if raw data changes |
...
| E03 | clean/crops.R | ag3b_01b == 2 filters short-season plots | unclear — FLAG | Confirm codebook meaning; count excluded plots; profile by crop type |
...
| E07 | clean/milk.R | physiological plausibility thresholds for milk | unclear — FLAG | Profile sensitivity to thresholds; compare excluded vs included |
...
| E09 | clean/destinations.R | ±30% tolerance on disposition vs harvest | unclear — FLAG | Sensitivity to threshold (20%, 30%, 40%); profile excluded by crop |
| E10 | clean/destinations.R | "crop produces no residue" records dropped | unclear — FLAG | Confirm survey category vs data-entry artefact; check double space in filter string |
...
| C06 | clean/destinations.R | residue section depends on mass_crops.rds | mass_crops.rds from Section 3 of same script | 04_build_households.R (run Section 4 after Section 3) |
...
| 🔴 High | A10: Confirm whether Alexander.2016 EW includes offal (double-count risk in MFA) | Literature |
