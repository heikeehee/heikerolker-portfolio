...
This file contains information on crop production and links production info to plot information.

Initially the link to plotsize was an exclusion criterium (i.e., if plotsize is smaller than area harvested the household is assumed to be an "inaccurate reporter/unreliable"), I abandoned this criterium to only maintain the ones that directly relate to the masses produced/used. This means the current file joins crop and plot information but does not save the resulting files nor uses the information of the merge in any form (not for exclusion (coded in "mismatch") or other).


Final files:
 
 **crops**/**trees**: production quantities for each crop and season, does not contain plot information
 **plots_stats**: number of plots and size owned by each household
 **plots**: detailed ownership and size of each plot owned
 
 No uncertainty calculations.

# Plots

Combine plot information from long and short rainy season.

```{r plots hk}
# load data
ag_sec_2a <- read_dta(here::here("data", "raw", "lsms","ag_sec_2a.dta")) # --> long rainy
ag_sec_2b <- read_dta(here::here("data", "raw", "lsms","ag_sec_2b.dta")) # --> short rainy
...
-   prepare plots for merge with crops and trees

*Converting area to ha reconcile farmers estimates and gps measures.*
...
→ empty plotnums will disappear with merge, they are irrelevant in terms of missing data

Aim:

-   determine total plot area by household (no exclusions!)

-   divide household land ownership in tertiles --\> done at descriptive, no need here

    *Includes a number of area estimates (gps and farmer).*
...
# calculate total land owned and/or cultivated by household
...
Crop production information for long and short rainy seasons are combined.

```{r crops hk}
# load data
ag_sec_4a <- read_dta(here::here("data", "raw", "lsms","ag_sec_4a.dta")) # --> long rainy
ag_sec_4b <- read_dta(here::here("data", "raw", "lsms","ag_sec_4b.dta")) # --> short rainy
...
-   determine production volumes for each crop and plot
...
saveRDS(crops_prelost, here::here("data", "processed", "01", "crops_prelost.RDS"), compress = T)
```
...
ag_sec_6a <- read_dta(here::here("data", "raw", "lsms","ag_sec_6a.dta")) # --> fruit
ag_sec_6b <- read_dta(here::here("data", "raw", "lsms","ag_sec_6b.dta")) # --> permanent
...
-   determine production at each plot and number of plants

-   calculate tree density per plotsize (old, not informative)
...
### Pre-harvest losses/yield gap

Aim (BONUS):

-   determine number of plots with preharvest losses

-   estimate area not harvested due to loss

```{r preharvest losses, eval=FALSE}
prec <- pc[,.(y4_hhid, plotnum, type, cropid, plotsize, preharvest_losses, loss_cause, harvested, lessharvest)]
```

Two households with no harvest quant and almost all else missing need to be removed --\> marked just as the plot mismatch households.

Aim:

-   join crops & trees for pre-harvest losses, keep plot info
...
- change food groups to FCT groups for consistency across,  maybe not strictly necessary
