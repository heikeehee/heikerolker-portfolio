...
# Adjusting food groups
newfoodgr <- readRDS("~/Documents/2index/2index/data/c4/LSMS_items_match_fin.RDS") %>% 
...
dt <- crops %>% 
  inner_join(incl_3b, by = "y4_hhid") %>%  # remove excluded households
  select(-type) %>% 
  left_join(newfoodgr, by = "item") %>% 
  relocate(type, .before = "item") %>% 
  mutate(type = ifelse(is.na(type), "cashcrops", type)) %>% # based on list on p36
  rowwise() %>% 
  mutate(
  ...
  group_by(y4_hhid, type) %>% # this shoudl be new_name_ag and group from lsms_items_match
...
fwrite(dt, "data/c4/mfa_crops_cleanedc4.csv")
...
# Save simulation results to disk (to avoid re-running on large datasets)
# fwrite(simulation_results, "data/c4/simulation_results_cropsc4.csv")

### Aggregation to household----
aggregated_results <- aggregate_simulation_results(simulation_results)

fwrite(aggregated_results, "data/c4/aggregated_results_cropsc4.csv")
...
### Aggregate----
aggregated_results <- aggregate_simulation_results(simulation_results)
fwrite(aggregated_results, "data/c4/aggregated_results_apc4.csv")
...
fwrite(dt, "data/c4/mfa_meat_cleanedc4.csv")
...
### Aggregate----
aggregated_results <- aggregate_simulation_results(simulation_results)
fwrite(aggregated_results, "data/c4/aggregated_results_meatc4.csv")
...
household_mcsc4 <- rbind(household_ap, household_crops, household_meat)
fwrite(household_mcsc4, "data/c4/household_mcsc4.csv")
